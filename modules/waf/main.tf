# Resources for the AWS Web Application Firewall Terraform module.
#
# This module only handles "CLOUDFRONT" scoped firewalls, which is currently
# our only use-case.

locals {
  name        = "${var.key}-${var.name}-${var.environment}"
  ipv4_ip_set = "${var.key}-waf-ipv4-blacklist-${var.environment}"
  ipv6_ip_set = "${var.key}-waf-ipv6-blacklist-${var.environment}"
  scope       = "CLOUDFRONT"
}

resource "aws_wafv2_ip_set" "ipv4" {
  name               = local.ipv4_ip_set
  scope              = local.scope
